<template>
  ...
  <div>
    ...
    <!-- 添加弹出框，通过遍历salaryItemName数组，来依次添加数据 -->
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="50%">
      ...
      >
        ...
        <!-- v-model="salary[title]"为salary中的key值，value为key对应的value的值 -->
        <!-- v-show="activeItemIndex == index"只展示输入的activeItemIndex与index相匹配的项 -->
        <el-input
...
export default {
  ...
  data() {
    return {
      ...
      dialogTitle: "添加工资账套",
      salaries: [],
      // salary中的项共计11项，但由于从0开始计数，故activeItemIndex中的值最大为10
      activeItemIndex: 0,
      salaryItemName: [
        "基本工资",
        "交通补助",
  ...
  methods: {
    ...
    showEditSalaryView(data) {
      // 不能通过this.salary=data来赋值，因为，salary里面的属性顺序和data的属性顺序不一样
    ...
    preStep() {
      if (this.activeItemIndex == 0) {
        ...
        // 输入至最后一项，但不想添加，上一步按钮更改为取消，并关闭对话框
        this.dialogVisible = false;
        this.activeItemIndex = 0;
        return;
      }
    ...
    // 复用弹出框，判断是添加操作还是修改操作，若有id则为修改，若无id则为添加
    nextStep() {
